  ...
  "client" should {
    "insert data" in withContainers { implicit clickHouseContainer =>
      ...
      withConnection { connection =>
        ...
        statement.execute(
          "create table test.users (name text, age Int8) Engine = MergeTree() order by name"
        )
      }
      ...
      val inserted = withConnection { connection =>
        val statement = connection.createStatement()
        val resultSet =
          statement.executeQuery("select name, age from test.users")
    ...
    "insert data via a statement containing reserved URI characters" in withContainers {
      implicit clickHouseContainer =>
        withConnection { connection =>
          val statement = connection.createStatement()
          statement.execute("create database if not exists test")
          statement.execute(
            "create table test.reserved_chars_users (name text, age Int8) Engine = MergeTree() order by name"
          )
        }
        ...
        val inserted = withConnection { connection =>
          ...
          val resultSet = statement.executeQuery(
            "select name, age from test.reserved_chars_users"
          )
          val buffer = scala.collection.mutable.ArrayBuffer.empty[User]
          while (resultSet.next())
            buffer += User(resultSet.getString("name"), resultSet.getInt("age"))
    ...
    "insert data spanning multiple batches" in withContainers {
      implicit clickHouseContainer =>
        ...
        withConnection { connection =>
          ...
          statement.execute("create database if not exists test")
          statement.execute(
            "create table test.batched_users (name text, age Int8) Engine = MergeTree() order by name"
          )
        }
        ...
        fs2.Stream
          ...
          .flatMap(clickhouse =>
            fs2.Stream
              ...
              .through(
                clickhouse.insert[User](
                  "insert into test.batched_users",
                  maxBatchSize = 37
                )
              )
          )
        ...
        val inserted = withConnection { connection =>
          val statement = connection.createStatement()
          val resultSet =
            statement.executeQuery("select name, age from test.batched_users")
          val buffer = scala.collection.mutable.ArrayBuffer.empty[User]
          while (resultSet.next())
            buffer += User(resultSet.getString("name"), resultSet.getInt("age"))
          buffer.toVector
